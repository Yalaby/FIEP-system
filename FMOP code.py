...
def studs_import(collection, file, sheet_name, change):
    duplicate_list=list()
    delstud_list=list()
    try:
        df2=pnd.read_excel(file, sheet_name=sheet_name)
        deletespaces(df2)
...
def marklists_import(marklistcollection, studcollection, deldutiescollection, file, sheet_name):
    try:
        df=pnd.read_excel(file, sheet_name=sheet_name)
        deletespaces(df)
...
tab1_layout = [
    [sg.InputText(key="Sheetname_groups"),sg.Text("Импортируемый лист")],
    [sg.Button("Импортировать excel с данными студентов"),
    sg.Checkbox('Изменять данные об уже существующих студентах', default=False,key="Change")],
...
while True:
    ...
    if event == "Импортировать excel с данными студентов":
        ...
        if (values["Sheetname_groups"] and filename):
            window.FindElement('Out').Update('')
            print(errorexplain(studs_import(stud_collection, filename, values["Sheetname_groups"], values['Change'])))
            fullrefresh(window, stud_collection, stud_deleted_collection)
        else: 
            window.FindElement('Out').Update('')
            print("Введите имя листа с группами!")
    if event == "Импортировать excel с оценками студентов":
        filename=askopenfilename()
        if (values["Sheetname_groups"] and filename):
            window.FindElement('Out').Update('')
            print(errorexplain(marklists_import(duty_collection, stud_collection, duties_deleted_collection, filename, values["Sheetname_groups"])))
            window.FindElement('flbox').Update(sorted(set(getlist(stud_collection, 'faculty'))))
            window.FindElement('smlbox').Update(list(set(smlboxlist())))
            fullrefresh(window, stud_collection, stud_deleted_collection) 
        else: 
